...
__global__ void
matrixMulCUDA3(float *C, float  *B, float *A, int n)
{
...
__global__ void
matrixMulCUDA4(float *C, float *A, float *B, unsigned int n)
{
	/*
...
__global__ void
matrixMulCUDA5(float *C, float *A, float *B, unsigned int n)
{
...
void sequentialMatMul(float *C, float *B, float *A, unsigned int n)
{
...
float computeMeanSquaredError(float *A, float *B, unsigned int n)
{
	float sumOfError = 0;
	for (int i = 0; i < n; i++) {
		float deviation = A[i] - B[i];
		deviation *= deviation;
		sumOfError += deviation;
	}
...
int matrixMultiply(int argc, char **argv, int n)
{
	...
	// Allocated for sequential computation
	float *h_X = new float[mem_size_C];
	...
	dim3 gridSize(ceil((float)n/32), ceil((float)n/32), 1);
	...
	// Execute the kernel
	matrixMulCUDA5 << < gridSize, blockSize >> > (d_C, d_A, d_B, n);
	...
	// Call serial kernel :D
	sequentialMatMul(h_X, h_B, h_A, n);

	// Compute the Mean Squared Error
	float MSE = computeMeanSquaredError(h_X, h_C, n);

	printf("Computed MSE: %lf\n", MSE);
