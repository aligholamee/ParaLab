...
__global__ void
reduceKernel2(int *g_inData, int *g_outData, int inSize, int outSize)
{
	extern __shared__ int sData[];
...
__global__ void
reduceKernel3(int *g_inData, int *g_outData, int inSize, int outSize)
{
	extern __shared__ int sData[];
...
int parallelReduction(int *a)
{
	...
	// KERNEL LAUNCH
	reduceKernel << <gridDimensions, blockDimensions, block_x >> > (d_A, d_Blocks, memSize, blocksMemSize);
	...
	funcCheck(cudaMemcpy(h_Blocks, d_A, memSize, cudaMemcpyDeviceToHost));
	printf("Parallel computation result: \n");
	printArray(h_Blocks, blocksMemSize);
