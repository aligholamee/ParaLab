...
#define funcCheck(stmt) do { cudaError_t err = stmt; if (err != cudaSuccess) { printf("Failed to run stmt %d ", __LINE__); return -1; } } while (0)
...
__global__ void
reduceKernel(int *g_inData, int *g_outData)
{
...
int parallelReduction(int *a)
{
	...
	// KERNEL LAUNCH
	reduceKernel << <gridDimensions, blockDimensions >> > (d_A, d_Blocks);
	...
	funcCheck(cudaMemcpy(a, d_A, memSize, cudaMemcpyDeviceToHost));
